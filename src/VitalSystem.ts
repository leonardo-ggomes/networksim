...
// showInstruction migrado para Phone.inbox.push
...
export default class VitalSystem {
    ...
    private recharge(delta: number) {
        ...
        if (this.cooldownRemaining > 0) {
            ...
            window.Phone?.chat.receive('ctOS', '🔋 Dispositivo recarregado!');
        }
    ...
    private drainEnergy(delta: number) {
        ...
        if (infoPlayer.energy <= 20 && !this.warnedEnergy) {
            ...
            window.Phone?.chat.receive('ctOS', '🔋 Bateria baixa — sente-se para recarregar!');
        }
        ...
        if (infoPlayer.energy <= 0) {
            ...
            if (this.cooldownRemaining <= 0) {
                ...
                window.Phone?.inbox.push({
                    id:    '__si_1',
                    title: '🔋 Bateria esgotada',
                    body:  'Sente-se para recarregar o dispositivo.',
                    type:  'info',
                });
                window.Phone?.chat.receive('ctOS', '🔋 Terminal bloqueado! Sente-se para recarregar.');
            }
        }
    }
    ...
    private drainHealth(delta: number) {
        ...
        if (infoPlayer.health <= CFG.healthDangerZone && !this.warnedHealth) {
            ...
            window.Phone?.chat.receive('ctOS', '❤️ Vida crítica — sente-se para recuperar!');
            window.Phone?.inbox.push({
                id:    '__si_2',
                title: '❤️ Saúde baixa',
                body:  'Sente-se em uma cadeira para recuperar.',
                type:  'info',
            });
        }
    ...
    private tickCooldown(delta: number) {
        ...
        if (this.cooldownRemaining <= 0) {
            ...
            window.Phone?.chat.receive('ctOS', '🔋 Dispositivo recarregado — terminal disponível.');
        }
    }
    ...
    private onDeath() {
        ...
        window.Phone?.chat.receive('ctOS', `💀 Você desmaiou! -$${CFG.deathMoneyPenalty} e vida restaurada a ${CFG.deathHealthReset}%.`);
        window.Phone?.inbox.push({
            id:    '__si_3',
            title: '💀 Desmaiou',
            body:  `Você perdeu $${CFG.deathMoneyPenalty}. Sente-se para recuperar.`,
            type:  'info',
        });
    }
